...
simple_calculator = Entry(root, width=35, borderwidth=5)
simple_calculator.grid(row=0, column=0, columnspan=3, padx=10, pady=10)


def button_click(number):
    current = simple_calculator.get()
    simple_calculator.delete(0, END)
    simple_calculator.insert(0, str(current) + str(number))
...
def button_add():
    first_number = simple_calculator.get()
    global f_num
    global math
    math = "addition"
...
def button_equal():
    second_number = simple_calculator.get()
    simple_calculator.delete(0, END)
    if math == "addition":
        simple_calculator.insert(0, f_num + int(second_number))
